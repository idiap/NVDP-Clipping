...
# SPDX-FileCopyrightText:  Aleksei Triastcyn
...
from scaled_renyi import scaled_renyi_divergence
...
class BayesianPrivacyAccountant:
    ...
    def __init__(self, powers=32, total_steps=1, scaled_renyi_fn=None, conf=1-1e-16, bayesianDP=True):
        ...
        self.scaled_renyi_fn = scaled_renyi_divergence if scaled_renyi_fn is None else scaled_renyi_fn
        self.conf = conf
        self.bayesianDP = bayesianDP
        # total accumulated privacy costs (log of moment generating function) per Renyi order
        self.privacy_cost = np.zeros_like(self.powers, dtype=np.float64)
    ...
    def _compute_bdp(self, power, scaled_renyi_fn, ldistr, rdistr, q, steps):
        ...
        logmgf_samples = self.holder_correction * steps * torch.max(c_L, c_R).detach().numpy()
        ...
        if np.std(logmgf_samples) < np.finfo(np.float64).eps:
            ...
            log_mgf_std = 0.5 * (2 * max_logmgf - np.log(n_samples) +\
                                 np.log(np.sum( np.where(np.isclose(np.exp(2 * logmgf_samples - 2 * max_logmgf), 1.0, atol=1e-8), 1.0, np.exp(2 * logmgf_samples - 2 * max_logmgf)) -\
    ...
    def _log_binom_expect(self, n, p, scaled_renyi_fn, ldistr, rdistr):
        ...
        # n should be int
        log_binom_coefs = torch.tensor(binom.logpmf(k, n=int(n), p=p))
