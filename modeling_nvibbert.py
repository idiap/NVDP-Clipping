...
@dataclass
class BaseModelOutputWithPastAndCrossAttentionsAndKL(ModelOutput):
    """Base class for model's outputs that may also contain a past key/values (to speed up
    sequential decoding).
...
@dataclass
class BaseModelOutputWithPoolingAndCrossAttentionsAndKL(ModelOutput):
...
@dataclass
class SequenceClassifierOutputAndKL(SequenceClassifierOutput):
...
class BertDenoisingSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None, nvib_dict=None):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(
            config, "embedding_size"
        ):
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            # NOTE: Here is standard self attention. We use the key and value from nvib
        ...
        # NOTE: Removed relative query and key embeddings for now
        ...
        if True:
            ...
            # Take the dot product between "query" and "key" to get the raw attention scores.
            # Dimension of attention_scores: (bsz, self.num_heads, tgt_len, src_len)
            attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2)) / math.sqrt(
                self.attention_head_size
            )

            ################
            # pi (bsz, self.num_heads, 1, src_len)
            pi = (
                nvib_tuple[1]
                .unsqueeze(1)
            ...
            pi = torch.clamp(pi.clone(), min=torch.finfo(
                pi.dtype).tiny)  # [B, S, 1]
            ...
            l2_norm = (1 / (2 * math.sqrt(self.attention_head_size))) * (
                (torch.norm(Z, dim=2)).unsqueeze(2) ** 2
            )
...
## this is the first cross attention 
...
class BertAttention(nn.Module):
    def __init__(self, config, accountant=None, position_embedding_type=None):
        ...
        # Check if we should add an NVIB layer
        if config.nvib_layers.pop(0):
            ...
            self.nvib_layer = Nvib(
                ...
                prior_var=config.prior_vars.pop(
                    0) if config.prior_var else None,
                prior_log_alpha=(
                    config.prior_log_alphas.pop(
                        0) if config.prior_log_alphas else None
                ),
                prior_log_alpha_stdev=(
                    config.prior_log_alpha_stdevs.pop(
                        0) if config.prior_log_alpha_stdevs else None
                ),
                delta=config.delta,
                # kappa=config.kappa,
                nheads=config.num_attention_heads,
                alpha_tau=config.alpha_tau,
                mu_tau=config.mu_tau,
                stdev_tau=config.stdev_tau,
                learnable_prior=config.learnable_prior,
                perform_latent_clipping= config.perform_latent_clipping, 
                max_latent_norm = config.max_latent_norm,  
                max_alpha = config.max_alpha,
            )
            self.self = BertDenoisingSelfAttention(
    ...
    def forward(
        ...
        # Denoising Self Attention #
        ############################
        # Convert attention mask from (batch, 1, tgt_len, src_len) -> (batch, src_len) boolean
        if hasattr(self, "nvib_layer"):
            # tuple = (z, pi, mu, logvar, alpha, mask)
            latent_tuple = self.nvib_layer(
                hidden_states, attention_mask.bool().squeeze(1)[:, 0, :]
            )
            attention_mask = (
                latent_tuple[5].unsqueeze(1).unsqueeze(
                    1).repeat(1, 1, attention_mask.size(2), 1)
            ...
            kl_gaussian = self.nvib_layer.kl_gaussian(
                mu=latent_tuple[2],
                logvar=latent_tuple[3],
                alpha=latent_tuple[4],
                memory_key_padding_mask=latent_tuple[5],
            )
            ...
            renyi_divergence = self.nvib_layer.renyi_divergence(
                mu = latent_tuple[2],
                logvar = latent_tuple[3],
                alpha=latent_tuple[4],
            )
        ...
        # Add the kl divergences to the outputs at the end
        if hasattr(self, "nvib_layer"):
...
## this is the second cross attention 
...
class BertLayer(nn.Module):
    def __init__(self, config, accountant=None):
        ...
        self.seq_len_dim = 1
        self.attention = BertAttention(config, accountant)
        self.is_decoder = config.is_decoder
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = BertAttention(
                config, accountant, position_embedding_type="absolute")
...
class BertEncoder(nn.Module):
    def __init__(self, config, accountant=None):
        ...
        self.layer = nn.ModuleList([BertLayer(config, accountant)
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            # CHeck if layer has nvib
            if hasattr(layer_module.attention, "nvib_layer"):
                # Get the KL divergences from the layer outputs
                #breakpoint()
                all_renyi_divergence = all_renyi_divergence + (layer_outputs[-3],)
                all_kl_gaussian = all_kl_gaussian + (layer_outputs[-2],)
                all_kl_dirichlet = all_kl_dirichlet + (layer_outputs[-1],)
...
class BertModelNVIB(BertPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.config.nvib_layers = [
            i in self.config.nvib_layers for i in range(self.config.num_hidden_layers)
        ]
    ...
    ) -> Union[Tuple[torch.Tensor], BaseModelOutputWithPoolingAndCrossAttentionsAndRenyi]:
        r"""encoder_hidden_states  (`torch.FloatTensor` of shape `(batch_size, sequence_length,
        hidden_size)`, *optional*): Sequence of hidden-states at the output of the last layer of
        the encoder. Used in the cross-attention if the model is configured as a decoder.
        encoder_attention_mask (`torch.FloatTensor` of shape `(batch_size, sequence_length)`,
        *optional*): Mask to avoid performing attention on the padding token indices of the encoder
        input. This mask is used in the cross-attention if the model is configured as a decoder.
        ...
        return BaseModelOutputWithPoolingAndCrossAttentionsAndRenyi(
            ...
            kl_gaussian=encoder_outputs.kl_gaussian,
            kl_dirichlet=encoder_outputs.kl_dirichlet,
            renyi_divergence=encoder_outputs.renyi_divergence,
        )
...
class BertForPreTraining(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = BertModelNVIB(config)
    ...
    def forward(
        ...
        for computing the masked language modeling loss. Indices should be in `[-100, 0, ...,
        config.vocab_size]` (see `input_ids` docstring) Tokens with indices set to `-100` are
        ignored (masked), the loss is only computed for the tokens with labels in `[0, ...,
        config.vocab_size]` next_sentence_label (`torch.LongTensor` of shape `(batch_size,)`,
        *optional*): Labels for computing the next sequence prediction (classification) loss. Input
...
class BertLMHeadModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = BertModelNVIB(config, add_pooling_layer=False)
...
class BertForNextSentencePrediction(BertPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        r"""Labels (`torch.LongTensor` of shape `(batch_size,)`, *optional*): Labels for computing
        the next sequence prediction (classification) loss. Input should be a sequence pair (see
...
class BertForSequenceClassificationNVIB(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(
        ...
        #klg_loss2 = outputs.kl_gaussian.mean()
        #kld_loss2 = outputs.kl_dirichlet[0].mean()
        klg_loss = torch.stack(outputs.kl_gaussian, dim=0).mean()
        kld_loss = torch.stack(outputs.kl_dirichlet, dim=0).mean()
        renyi_divergence = (torch.stack(outputs.renyi_divergence, dim=0).max(), torch.stack(outputs.renyi_divergence, dim=0).mean())
        ...
        if labels is not None and return_loss:
            ...
            if self.config.NVIB:
                #loss = task_loss + klg_loss + kld_loss # add a weight on the sum of klg_loss and kld_loss 
        ...
        ''' 
        return SequenceClassifierOutputAndKL(
            loss=loss,
            logits=logits,
            hidden_states=outputs.hidden_states,
            attentions=outputs.attentions,
            kl_gaussian=outputs.kl_gaussian,
            kl_dirichlet=outputs.kl_dirichlet,
        )
        ''' 
        ...
        ) if loss is not None else SequenceClassifierOutputAndRenyi(logits=logits) # for predictions
